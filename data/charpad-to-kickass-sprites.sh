...
# Prepend ".const " to lines starting with a letter that have an equals
awk -v var="$base_name" '/^[A-Za-z].*=/{printf ".const %s_%s\n", var, $0; next} {print}' "$sprites_file" > temp_file && mv temp_file "$sprites_file"
...
sed -i "s/sprite_image/${base_name}_sprite_image/g" "$sprites_file"
sed -i "s/spriteset/${base_name}_spriteset/g" "$sprites_file"